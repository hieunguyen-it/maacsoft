
				
				<!-- Main container start -->
				<div class="main-container">

					<!-- Row starts -->
					<div class="row gutters">
						<div class="col-xl-8 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="card h-350">
								<div class="card-header">
									<div class="card-title">Weekly Sales</div>
								</div>
								<div class="card-body">
									<div class="chartist bar-scheme-one">
										<div class="stacked-bar"></div>
									</div>
									<!-- Row starts -->
									<div class="row gutters">
										<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-6">
											<div class="info-stats3 shade-one-a">
												<i class="icon-turned_in"></i>
												<h6>Visitors</h6>
												<h3>4500</h3>
											</div>
										</div>
										<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-6">
											<div class="info-stats3 shade-one-b">
												<i class="icon-turned_in"></i>
												<h6>Online Orders</h6>
												<h3>3500</h3>
											</div>
										</div>
										<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-12">
											<div class="info-stats3 shade-one-c">
												<i class="icon-turned_in"></i>
												<h6>Offline Orders</h6>
												<h3>2000</h3>
											</div>
										</div>
									</div>
									<!-- Row end -->
								</div>
							</div>
						</div>
						<div class="col-xl-4 col-lg-12 col-md-12 col-sm-12 col-12">
							<!-- Row starts -->
							<div class="row gutters">
								<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
									<div class="card h-167">
										<div class="card-body">
											<div class="chartist line-scheme-one">
												<div class="line-chart-visitors"></div>
											</div>
											<div class="info-stats2">
												<div class="details">
													<h3>4000</h3>
													<h6>Visitors</h6>
												</div>
											</div>
										</div>
									</div>
									<div class="card h-167">
										<div class="card-body">
											<div class="chartist line-scheme-one">
												<div class="line-chart-likes"></div>
											</div>
											<div class="info-stats2">
												<div class="details">
													<h3>7000</h3>
													<h6>Likes</h6>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
									<div class="card h-167">
										<div class="card-body">
											<div class="chartist line-scheme-one">
												<div class="line-chart-orders"></div>
											</div>
											<div class="info-stats2">
												<div class="details">
													<h3>8000</h3>
													<h6>Orders</h6>
												</div>
												<div class="notification" data-toggle="tooltip" data-placement="left" title="" data-original-title="5 New Orders">
													<i class="icon-warning text-success"></i>
												</div>
											</div>
										</div>
									</div>
									<div class="card h-167">
										<div class="card-body">
											<div class="chartist line-scheme-four">
												<div class="line-chart-reviews"></div>
											</div>
											<div class="info-stats2">
												<div class="details">
													<h3>3000</h3>
													<h6>Reviews</h6>
												</div>
												<div class="notification" data-toggle="tooltip" data-placement="left" title="" data-original-title="5 Best Reviews">
													<i class="icon-warning text-danger"></i>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<!-- Row ends -->
						</div>
					</div>
					<!-- Row ends -->

					<!-- Row starts -->
					<div class="row gutters">
						<div class="col-xl-8 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="card h-320">
								<div class="card-header">
									<div class="card-title">Sales by Location</div>
								</div>
								<div class="card-body">
									<div id="world-map-markers" class="chart-height-sm"></div>
									<!-- Row starts -->
									<div class="row gutters">
										<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-4">
											<div class="info-stats3 shade-two-a">
												<i class="icon-map1"></i>
												<h6>America</h6>
												<h3>$6800</h3>
											</div>
										</div>
										<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-4">
											<div class="info-stats3 shade-two-b">
												<i class="icon-map1"></i>
												<h6>Asia</h6>
												<h3>$9200</h3>
											</div>
										</div>
										<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-4">
											<div class="info-stats3 shade-two-c">
												<i class="icon-map1"></i>
												<h6>Eurpoe</h6>
												<h3>$3500</h3>
											</div>
										</div>
									</div>
									<!-- Row end -->
								</div>
							</div>
						</div>
						<div class="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="card h-320">
								<div class="card-header">
									<div class="card-title">Income</div>
								</div>
								<div class="card-body">
									<div class="income-stats-container">
										<div class="income-stats">
											<div class="income-graph">
												<div id="overallSales"></div>
											</div>
											<div class="income-info">
												<h3>38<sup>k</sup><i class="icon-chevron-up text-success"></i></h3>
												<p>Overall Sales</p>
											</div>
										</div>
										<div class="income-stats">
											<div class="income-graph">
												<div id="overallExpenses"></div>
											</div>
											<div class="income-info">
												<h3>24<sup>k</sup><i class="icon-chevron-up text-success"></i></h3>
												<p>Overall Expenses</p>
											</div>
										</div>
										<div class="income-stats">
											<div class="income-graph">
												<div id="overallIncome"></div>
											</div>
											<div class="income-info">
												<h3>14<sup>k</sup><i class="icon-chevron-down text-danger"></i></h3>
												<p>Overall Income</p>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="card h-320">
								<div class="card-header">
									<div class="card-title">Todo's</div>
								</div>
								<div class="card-body">
									<div class="todo-container">
										<ul class="todo-body">
											<li class="todo-list">
												<div class="todo-info">
													<span class="dot blue"></span>
													<p>Team Meeting</p>
													<p>Thursday at 3:30 PM</p>
												</div>
											</li>
											<li class="todo-list done">
												<div class="todo-info">
													<span class="dot orange"></span>
													<p>Make new page</p>
													<p>Wednesday at 10:30 AM</p>
												</div>
											</li>
											<li class="todo-list done">
												<div class="todo-info">
													<span class="dot yellow"></span>
													<p>Product launch</p>
													<p>Sunday at Baur Lac, Zurich</p>
												</div>
											</li>
											<li class="todo-list done">
												<div class="todo-info">
													<span class="dot green"></span>
													<p>Code Review</p>
													<p>Friday at 2:00PM</p>
												</div>
											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
						<div class="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="card h-320">
								<div class="card-body">
									<div class="agenda">
										<div class="todays-date">
											<h5>Agenda - <span class="date" id="todays-date"></span></h5>
										</div>
										<ul class="agenda-list">
											<li>
												<span class="bullet">&nbsp;</span>
												<div class="details">
													<p>Call with Sayed</p>
													<small>09:00 am</small>
												</div>
											</li>
											<li>
												<span class="bullet">&nbsp;</span>
												<div class="details">
													<p>Book a Flight to California</p>
													<small>09:30 am</small>
												</div>
											</li>
											<li>
												<span class="bullet">&nbsp;</span>
												<div class="details">
													<p>Standup Meeting</p>
													<small>10:00 am</small>
												</div>
											</li>
											<li>
												<span class="bullet secondary">&nbsp;</span>
												<div class="details">
													<p>Talk to Aaleyah</p>
													<small>11:00 am</small>
												</div>
											</li>
											<li>
												<span class="bullet secondary">&nbsp;</span>
												<div class="details">
													<p>Project Discussion</p>
													<small>12:30 pm</small>
												</div>
											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
						<div class="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="card h-320">
								<div class="card-header">
									<div class="card-title">Overall Ratings</div>
								</div>
								<div class="card-body">
									<div class="user-ratings">
										<div class="total-ratings">
											<h2>4.5</h2>
											<div class="rating-stars">
												<div id="rate1"></div>
											</div>
										</div>
										<div class="ratings-list-container">
											<div class="ratings-list">
												<div class="rating-level">5.0</div>
												<div class="rating-stars">
													<div class="rateA"></div>
												</div>
												<div class="total">
													8,500 <span class="percentage">65%</span>
												</div>
											</div>
											<div class="ratings-list">
												<div class="rating-level">4.0</div>
												<div class="rating-stars">
													<div class="rateB"></div>
												</div>
												<div class="total">
													3,500 <span class="percentage">20%</span>
												</div>
											</div>
											<div class="ratings-list">
												<div class="rating-level">3.0</div>
												<div class="rating-stars">
													<div class="rateC"></div>
												</div>
												<div class="total">
													1,400 <span class="percentage">15%</span>
												</div>
											</div>
											<div class="ratings-list">
												<div class="rating-level">2.0</div>
												<div class="rating-stars">
													<div class="rateD"></div>
												</div>
												<div class="total">
													300 <span class="percentage">05%</span>
												</div>
											</div>
											<div class="ratings-list">
												<div class="rating-level">1.0</div>
												<div class="rating-stars">
													<div class="rateE"></div>
												</div>
												<div class="total">
													75 <span class="percentage">03%</span>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<!-- Row ends -->

				</div>
				<!-- Main container end -->

			
		<!-- Page wrapper end -->

		<!--**************************
			**************************
				**************************
							Required JavaScript Files
				**************************
			**************************
		**************************-->
		<!-- Required jQuery first, then Bootstrap Bundle JS -->
		<script src="js/jquery.min.js"></script>
		<script src="js/bootstrap.bundle.min.js"></script>
		<script src="js/moment.js"></script>


		<!-- *************
			************ Vendor Js Files *************
		************* -->
		<!-- Slimscroll JS -->
		<script src="vendor/slimscroll/slimscroll.min.js"></script>
		<script src="vendor/slimscroll/custom-scrollbar.js"></script>

		<!-- Daterange -->
		<script src="vendor/daterange/daterange.js"></script>
		<script src="vendor/daterange/custom-daterange.js"></script>

		<!-- Chartist JS -->
		<script src="vendor/chartist/js/chartist.min.js"></script>
		<script src="vendor/chartist/js/chartist-tooltip.js"></script>
		<script src="vendor/chartist/js/custom/bar/stack-bar-chart.js"></script>
		<script src="vendor/chartist/js/custom/line/multiple-line-charts.js"></script>

		<!-- jVector Maps -->
		<script src="vendor/jvectormap/jquery-jvectormap-2.0.3.min.js"></script>
		<script src="vendor/jvectormap/world-mill-en.js"></script>
		<script src="vendor/jvectormap/gdp-data.js"></script>

		<!-- Custom JVector Maps -->
		<script src="vendor/jvectormap/custom/world-map-markers.js"></script>

		<!-- Circliful js -->
		<script src="vendor/circliful/circliful.min.js"></script>
		<script src="vendor/circliful/circliful.custom.js"></script>

		<!-- Rating JS -->
		<script src="vendor/rating/raty.js"></script>
		<script src="vendor/rating/raty-custom.js"></script>


		<!-- Main JS -->
		<script src="js/main.js"></script>

	